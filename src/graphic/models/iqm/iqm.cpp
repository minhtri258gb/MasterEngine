...
2014-03-30 RR: Make this IQM Compile working together with IqeBrower.
               * Replaced main() by IQM_Compiler() because IqeBrowser already
                 has a main function.
               * replaced fatal() by fatal2() and a return( EXIT_FAILURE).
                 This returns with an error and an error string to the calling program.
2014-04-01 RR: * Added: #include "IqmCompiler.h"
2014-04-08 RR: * Coded resetoutbuffers() and call this at begin of IQM_Compiler().
                 This resets all bufers to accumulate data for a model to write.
                 Whitout this, we would accumlate data by multiple calls of IQM_Compiler().
               * calcanimdata()
                 At begin of this function reset 'framesize' and 'animdata' to
                 support multiple calls of IQM_Compiler().
2014-05-01 RR: * Updated this source with changes from 'IQM Developer Kit 2013-10-02' to
                 'IQM Developer Kit 2014-03-15' release.
               * loadiqe()
                 Bugfix, reset mesh output data for animation files.
                 Only call smoothverts() and makemeshes() if there was meshes and
                 triangles in the file read in.
                 Per definition, only the first input file argument has meshes.
               * loadiqe()
                 * renamed to loadIqeOrIqm
                 * Is called for .iqe and .iqm files
2014-05-02 RR: * Added functions to load IQM models.
                 * parseiqm()
                   is the main function for reading IQM files.
                 * loadIqeOrIqm()
                   is the reworked loadiqe() function.
                   This funtion manage the load of IQE and IQM models.
                   This function must be called for .iqe and .iqm files.
               * makeanims()
                 Allow animation files without joints (or skelteon).
                 The change below setup the poses from joints of the first
                 file with joints. This can now be a file with meshes and
                 without animations.
                 Moved the line
                     if(eanims.empty()) return;
                 from before after the next statement group
...
bool loadiqmmeshes( const iqmheader &hdr, uchar *buf)
{
  ...
  const char *texts = hdr.ofs_text ? (char *)&buf[hdr.ofs_text] : "";
  ...
  for(int i = 0; i < (int)hdr.num_vertexarrays; i++)
    {
      ...
      switch(va.type) {
        case IQM_POSITION:
          ...
          lilswap(pInFloat, 3*hdr.num_vertexes);
          for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {
              epositions.add( Vec4(pInFloat[0], pInFloat[1], pInFloat[2], 1));
              pInFloat += 3;
            }
          break;
        case IQM_NORMAL:
          ...
          lilswap(pInFloat, 3*hdr.num_vertexes);
          for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {
              enormals.add( Vec3(pInFloat[0], pInFloat[1], pInFloat[2]));
              pInFloat += 3;
            }
          break;
        case IQM_TANGENT:
          ...
          pInFloat = (float *)&buf[va.offset];
          lilswap(pInFloat, 4*hdr.num_vertexes);
          for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {
              etangents.add( Vec4(pInFloat[0], pInFloat[1], pInFloat[2], pInFloat[3]));
              pInFloat += 4;
            }
          break;
        case IQM_TEXCOORD:
          ...
          pInFloat = (float *)&buf[va.offset];
          lilswap(pInFloat, 2*hdr.num_vertexes);
          for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {
              etexcoords.add( Vec4( pInFloat[0], pInFloat[1], 0.0, 0.0));
              pInFloat += 2;
            }
          break;
        ...
        case IQM_COLOR:
          if(va.format != IQM_UBYTE || va.size != 4) return false;
          pUByte = (uchar *)&buf[va.offset];
          for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {
              ecolors.add( Vec4( pUByte[0] / 255.0, pUByte[1] / 255.0, pUByte[2] / 255.0, pUByte[3] / 255.0));
              pUByte += 4;
            }
          break;
        }
  ...
  if( inblendindex != NULL && inblendweight != NULL)    // have blend data
    {

      for( iLoop = 0; iLoop < hdr.num_vertexes; iLoop++ ) {

          float total = inblendweight[0] + inblendweight[1] + inblendweight[2] + inblendweight[3];

          blendcombo b;

          b.addweight( inblendweight[0] / total, inblendindex[0]);
          b.addweight( inblendweight[1] / total, inblendindex[1]);
          b.addweight( inblendweight[2] / total, inblendindex[2]);
          b.addweight( inblendweight[3] / total, inblendindex[3]);
          b.finalize();
  ...
  pInInt = (int *)( buf + hdr.ofs_triangles);
    ...
    etriangles.add( etriangle( pInInt[0], pInInt[1], pInInt[2]));
  ...
  inmesh = (iqmmesh *)( buf + hdr.ofs_meshes);
	...
	for( iLoop = 0; iLoop < hdr.num_meshes; iLoop++ ) {
		...
		//x/inmesh[iLoop].name = LittleLong( inmesh[iLoop].name );
		//x/inmesh[iLoop].material = LittleLong( inmesh[iLoop].material );
		//x/inmesh[iLoop].first_vertex = LittleLong( inmesh[iLoop].first_vertex );
		//x/inmesh[iLoop].num_vertexes = LittleLong( inmesh[iLoop].num_vertexes );
		//x/inmesh[iLoop].first_triangle = LittleLong( inmesh[iLoop].first_triangle );
		//x/inmesh[iLoop].num_triangles = LittleLong( inmesh[iLoop].num_triangles );

    emeshes.add( emesh( getnamekey( texts + inmesh[iLoop].name),
                        getnamekey( texts + inmesh[iLoop].material),
                        inmesh[iLoop].first_triangle));
	}
  ...
  if( hdr.num_joints > 0) {
    ...
    joints = (iqmjoint *)( buf + hdr.ofs_joints);
      ...
      j.name  = getnamekey( texts + joints[iLoop].name);
      j.parent = joints[iLoop].parent;

		  transform t;

      t.pos = Vec3( joints[iLoop].translate[ 0], joints[iLoop].translate[ 1], joints[iLoop].translate[ 2]);
      loopk(3) t.orient[k] = joints[iLoop].rotate[ k];
      t.orient.restorew();
      double w = joints[iLoop].rotate[ 3];
      if(w != t.orient.w)
      {
        t.orient.w = w;
        t.orient.normalize();
      ...
      if(t.orient.w > 0) t.orient.flip();
      t.scale = Vec3( joints[iLoop].scale[ 0], joints[iLoop].scale[ 1], joints[iLoop].scale[ 2]);
	...
	if( hdr.num_comment > 0) {

	  for( iLoop = 0; iLoop < hdr.num_comment; iLoop++ ) {

      commentdata.add( buf[ hdr.ofs_comment + iLoop]);
	  }

	  if( buf[ hdr.ofs_comment + hdr.num_comment - 1] != 0) {
...
bool loadiqmanims( const iqmheader &hdr, uchar *buf)
{
  ...
  if( hdr.num_anims == 0) {     // security test, need animation. Is called only if with animation's
  ...
  // animation data
	...
	for( iLoop = 0; iLoop < hdr.num_anims; iLoop++ ) {
		...
		//x/anims[iLoop].name = LittleLong( anims[iLoop].name);
		//x/anims[iLoop].first_frame = LittleLong( anims[iLoop].first_frame);
		//x/anims[iLoop].num_frames = LittleLong( anims[iLoop].num_frames);
		//x/anims[iLoop].framerate = LittleFloat( anims[iLoop].framerate);
		//x/anims[iLoop].flags = LittleLong( anims[iLoop].flags);

    eanim &a = eanims.add();
    a.name       = getnamekey( texts + anims[iLoop].name);
    a.startframe = anims[iLoop].first_frame + eframes.length();
    a.endframe   = anims[iLoop].first_frame + anims[iLoop].num_frames + eframes.length();
    a.fps        = anims[iLoop].framerate;
    a.flags      = anims[iLoop].flags;
	}
#endif
    ...
    if( iAnimNumFrame <= 0) {      // start of a new animation

      iAnimIndex += 1;             // advance to next animation

      if( iAnimIndex >= (int)hdr.num_anims) {  // all done (security test, this may not happen)

        break;
      }

      eanim &a = eanims.add();
      a.name       = getnamekey( texts + anims[iAnimIndex].name);
      a.fps        = anims[iAnimIndex].framerate;
      a.flags      = anims[iAnimIndex].flags;
      a.startframe = eframes.length();
      if(!eframes.length() || eframes.last() != eposes.length()) eframes.add(eposes.length());

      iAnimNumFrame = anims[iAnimIndex].num_frames;    // # frames for this animation
      if( iAnimNumFrame <= 0) {                   // security test, may not be zero
    ...
    if(eanims.length() && eframes.length() && eframes.last() != eposes.length()) eframes.add(eposes.length());
      ...
      transform t;
			...
			t.pos[0] = poses[j].channeloffset[0]; if( poses[j].mask & 0x01 ) t.pos[0] += *framedata++ * poses[j].channelscale[0];
			t.pos[1] = poses[j].channeloffset[1]; if( poses[j].mask & 0x02 ) t.pos[1] += *framedata++ * poses[j].channelscale[1];
			t.pos[2] = poses[j].channeloffset[2]; if( poses[j].mask & 0x04 ) t.pos[2] += *framedata++ * poses[j].channelscale[2];

			t.orient[0] = poses[j].channeloffset[3]; if( poses[j].mask & 0x08 ) t.orient[0] += *framedata++ * poses[j].channelscale[3];
			t.orient[1] = poses[j].channeloffset[4]; if( poses[j].mask & 0x10 ) t.orient[1] += *framedata++ * poses[j].channelscale[4];
			t.orient[2] = poses[j].channeloffset[5]; if( poses[j].mask & 0x20 ) t.orient[2] += *framedata++ * poses[j].channelscale[5];
			t.orient.w  = poses[j].channeloffset[6]; if( poses[j].mask & 0x40 ) t.orient.w  += *framedata++ * poses[j].channelscale[6];
      if(t.orient.w > 0) t.orient.flip();

			t.scale[0] = poses[j].channeloffset[7]; if( poses[j].mask & 0x080 ) t.scale[0] += *framedata++ * poses[j].channelscale[7];
			t.scale[1] = poses[j].channeloffset[8]; if( poses[j].mask & 0x100 ) t.scale[1] += *framedata++ * poses[j].channelscale[8];
			t.scale[2] = poses[j].channeloffset[9]; if( poses[j].mask & 0x200 ) t.scale[2] += *framedata++ * poses[j].channelscale[9];

      eposes.add(t);
		}

    iAnimNumFrame -= 1;      // one more frame done
	}

  return true;
}

bool parseiqm(const char *filename, stream *f)
{
    uchar *buf = NULL;
    iqmheader hdr;
    ...
    if(hdr.num_meshes > 0 && !loadiqmmeshes( hdr, buf)) goto error;
    if(hdr.num_anims > 0 && !loadiqmanims( hdr, buf)) goto error;
...
error:
    //x/printf("%s: error while loading\n", filename);
...
bool loadIqeOrIqm(const char *filename, const filespec &spec)
{
    int numfiles = 0;
    while(filename)
    {
        ...
        const char *type = strrchr(file, '.');  // point to file type

        stream *f;

        if( type && !strcasecmp(type, ".iqm")) {   // is .iqm file

          f = openfile(file, "rb");        // read binary data file

        } else {                                   // must be .iqe file

          f = openfile(file, "r");         // read text data file
        }
        ...
        if(f)
        {
            ...
            if( type && !strcasecmp(type, ".iqm")) {   // is .iqm file
              ...
              if(parseiqm( file, f)) numfiles++;       // parse .iqm files

            } else {                                   // must be .iqe file

              if(parseiqe(f)) numfiles++;              // parse .iqe files
            }
            delete f;
        }
    ...
    if( emeshes.length())  // have meshes
    {
      ...
      makemeshes();        // make the output mesh
    }
    makeanims();
...
bool writeiqm(const char *filename)
{
    ...
    FILE *pFile;
    char *pAnimCfgFileName, *p;

    pAnimCfgFileName = (char *)malloc( strlen( filename) + 20);  // same more than the filename
    if( pAnimCfgFileName == NULL) {         // malloc failed

      return( false);
    }

    strcpy( pAnimCfgFileName, filename);    // copy filename

    p = strrchr( pAnimCfgFileName, '.');    // find last point
    if( p != NULL) {                        // found point
      *p = '\0';                            // set end of string
    }

    strcat( pAnimCfgFileName, "_animation.cfg");   // add suffix

    pFile = fopen( pAnimCfgFileName, "wt");         // oben text file
    if(!pFile)  {
      free( pAnimCfgFileName);
      return false;
    }

    // write file

    fprintf( pFile, "// animation config file\n");
    fprintf( pFile, "\n");
    fprintf( pFile, "\n");
    fprintf( pFile, "// animations in this file\n");
    fprintf( pFile, "// -----------------------\n");
    fprintf( pFile, "// first frame, num frames, looping frames, fps\n");

    loopv(anims)
    {
      anim &a = anims[i];

      fprintf( pFile, "// %11d, %10d, %14d, %3d  // %s\n",
        (int)a.firstframe,
        (int)a.numframes,
        (a.flags & IQM_LOOP) ? (int)a.numframes : 0,
        (int)(a.fps + 0.5),
        (char *)stringdata.getbuf() + a.name);
    }

    fprintf( pFile, "\n");
    fprintf( pFile, "\n");
    fprintf( pFile, "// joints of model\n");
    fprintf( pFile, "// ---------------\n");
    loopv(joints)
    {
        joint &j = joints[i];
...
static void fatal2( char *pResultString, const char *s, ...)    // failure exit
{
    defvformatstring(msg,s,s);
    strcpy( pResultString, msg);
}

int IQM_Compiler( int argc, char **argv, char *pResultString)
{

    if(argc <= 1) {
      strcpy( pResultString, "no arguments");
      return( EXIT_FAILURE);
    }

    // reset exporter accumulation buffers
    ...
    // reset some optional arguments

    escale = 1.0;          // scale output mesh
    ...
    if(!outfile) {
      fatal2( pResultString, "no output file specified");
      return( EXIT_FAILURE);
    } else if(infiles.empty()) {
      fatal2( pResultString, "no input files specified");
      return( EXIT_FAILURE);
    }
    ...
    loopv(infiles)
    {
      ...
      if(!type) {
        fatal2( pResultString, "no file type: %s", infile);
        return( EXIT_FAILURE);
      } else if(!strcasecmp(type, ".md5mesh"))
      {
        if(loadmd5mesh(infile, inspec)) conoutf("imported: %s", infile);
        else {
          fatal2( pResultString, "failed reading: %s", infile);
          return( EXIT_FAILURE);
        }
      }
      ...
      else if(!strcasecmp(type, ".iqe") || !strcasecmp(type, ".iqm"))
      {
        if(loadIqeOrIqm(infile, inspec)) conoutf("imported: %s", infile);
        else {
          fatal2( pResultString, "failed reading: %s", infile);
          return( EXIT_FAILURE);
        }
      ...
      else if(!strcasecmp(type, ".obj"))
      {
        ...
        fatal2( pResultString, "unknown file type: %s", type);
        return( EXIT_FAILURE);
      }
	}
  ...
  conoutf("");
    ...
    fatal2( pResultString, "failed writing: %s", outfile);
    return( EXIT_FAILURE);
	}
